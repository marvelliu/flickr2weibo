...
class API(object):
    ...
    def upload(self, filename, status, lat=None, long=None, source=None, image=None):
        if source is None:
            source=self.source
        headers, post_data = API._pack_image(filename, 1024, source=source, status=status, lat=lat, long=long, contentname="pic", image=image)
        args = [status]
...
#        )(self, *args, post_data=post_data, headers=headers)
    ...
    def _pack_image(filename, max_size, source=None, status=None, lat=None, long=None, contentname="image", image=None):
    
        """Pack image from file into multipart-formdata post body"""
        # image must be less than 700kb in size
        try:
            #if os.path.getsize(filename) > (max_size * 1024):
            if len(image) > (max_size * 1024):
        ...
        if file_type not in ['image/gif', 'image/jpeg', 'image/pjpeg', 'image/png']:
        ...
        body = [ bytes(x) for x in body]       
        
        #[ logging.info( "x %s %s "% (type(x), x)) for x in body]
